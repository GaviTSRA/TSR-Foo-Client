...
public abstract class InputHandler implements InputProcessor, GestureListener{
    ...
    public static void tileConfig(@Nullable Player player, Building build, @Nullable Object value){
        ...
        if(player != null){ // TO-DO: Move all this client stuff into the ClientLogic class
            if(Navigation.currentlyFollowing instanceof UnAssistPath path){
                if(path.assisting == player){
    ...
    public static void unitCommand(Player player){
        ...
        if(commander.isCommanding()){
            ...
            //TO-DO try out some other formations
    ...
    public void update(){
        ...
        if(controlledType != null && player.dead()){
            Unit unit = Units.closest(player.team(), player.x, player.y, u -> !u.isPlayer() && u.type == controlledType && !u.dead /* TO-DO: Make this a thing that actually works? && (!(u.controller() instanceof FormationAI f) || f.isBeingControlled(player.lastReadUnit)) */);
