...
public class NetClient implements ApplicationListener{
    ...
    public static void sendMessage(String message, String sender, Player playersender){
        ...
        if(ui != null) {
            ...
            ui.chatfrag.addMessage(message, sender, background);
            if (Core.settings.getBool("logmsgstoconsole") && net.client())
                Log.info("&fi@: @", "&lc" + (playersender == null ? "Server" : Strings.stripColors(playersender.name)), "&lw" + Strings.stripColors(message)); // Make sure we are a client, if we are the server it does this already
            }
