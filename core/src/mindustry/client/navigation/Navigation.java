...
public class Navigation {
    ...
    public static void update() {
        if (timer.get(600)) obstacles.clear(); // Refresh all obstacles every 600s since sometimes they don't get removed properly for whatever reason TO-DO: Check if this happens because it still runs update even when dead, if so just the removal of the obstacle

        if (targetPos != null && clientThread.taskQueue.size() == 0) { // must be navigating, T-ODO: dejank
...
//        Draw.color();
    ...
    public static void navigateTo(float drawX, float drawY) {
        if (Core.settings.getBool("assumeunstrict")) {
            NetClient.setPosition(Mathf.clamp(drawX, 0, world.unitWidth()), Mathf.clamp(drawY, 0, world.unitHeight())); // TO-DO: Detect whether or not the player is at their new destination, if not run with assumeunstrict off
