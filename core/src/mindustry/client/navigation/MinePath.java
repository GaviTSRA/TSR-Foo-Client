...
public class MinePath extends Path {
    ...
    public MinePath (String args){
        ...
        if (items.isEmpty()) {
            ...
            player.sendMessage(Core.bundle.format("client.path.miner.mining", itemString.substring(0, itemString.length() - 2))); // TO-DO: Terrible
        }
    }
    ...
    public void follow() {
        ...
        if (player.unit().maxAccepted(item) == 0) { // drop off
            ...
            player.boosting = player.unit().type.canBoost && !player.within(tile, tilesize * 2); // TO-DO: Distance based on formation radius rather than just moving super close
