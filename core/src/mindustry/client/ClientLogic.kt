 ...
 * TO-DO: Move the 9000 different bits of code throughout the client to here*/
class ClientLogic {
    ...
    init {
        ...
        Events.on(EventType.ClientLoadEvent::class.java) { // Run when the client finishes loading
            ...
            if (Core.settings.getInt("changeHash") != changeHash) {
                ChangelogDialog.show()
                Core.settings.put("updateurl", "GaviTSRA/TSR-Foo-Client")
            }
            ...
            Core.settings.getBoolOnce("client730") { Core.settings.put("disablemonofont", true) } // TO-DO: Remove later
        }
